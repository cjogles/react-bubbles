...
Answers:
1. A token is used for client side authentication. Attached to the header of an HTTP request,
the server is able to authenticate the client easily between each request without requesting
the password and username again and again. 

2. Use tokens to authenticate clients with axios. Run websites with as few privileges possible.
Use a firewall or something to keep you safe during the interim. Use cookies securely. Use 
HTTPS, use the x-xss-protection security header to prevent cross-site scripting attacks.
And it goes without saying, use strong passwords.

3. Web servers are computers that specialize in holding data and responding to other computer
requests for that data or some other data. The server processes these network requests via 
HTTP and several other related protocols. The primary purpose of a server is to store, process,
and deliver web pages to clients. 

4. CRUD acronym used to explain interface between client and API/servers can be explained 
with the HTTP methods of POST, GET, PUT, and DELETE. This can be done with the browsers
built in fetch interface or another library like axios. 
